...
class LevelBundleLoader {
    init() {
        self.levelBundles = Array<LevelBundle>()
        let levels = BoardLoader.loadLevels();
        self.loadTests(levels: levels)
        self.loadDiamonds(levels: levels)
    }
    ...
    private func loadTests(levels: [Level]) {
        ...
        self.levelBundles.append(demonstrations)
    }
